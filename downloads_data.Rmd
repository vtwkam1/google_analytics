...
download_page <- reactive({
    selected() %>%
        group_by(product) %>% 
        mutate(top_page_path = fct_lump_prop(page_path, w = downloads, prop = 0.1)) %>% 
        ungroup() %>% 
        group_by(product, year_month, top_page_path) %>% 
        summarise(monthly_downloads = sum(downloads))
})
```
...
renderPlotly({
    ...
    source_fig <- ggplot(data = download_page(),
                  aes(x = year_month, y = monthly_downloads, colour = product, linetype = top_page_path)) +
        geom_line() +
        labs(x = "Month",
             y = "Downloads")
        scale_y_continuous(expand = c(0,0),
