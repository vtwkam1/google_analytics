...
filename <- map(month_list, ~ sprintf("./download_analytics/HSC dashboard - Engagement and implementation_All webpages_ links and downloads_Table_%s.csv", as.character(.x) %>% str_remove("-01$")))
```
...
```{r, echo = FALSE}
renderDT(selected() %>% select(product, everything()),
         filter = "top",
         extensions = c("Scroller", "Buttons"),
         options = list(
             scrollY = 400,
